...
export function groupBy({
  ...
  for (const item of rows) {
    const groupKey = groupByConfig.fields.map((field) => getGroupingKey(item, field)).join('_');
...
function getGroupingKey(item: DataItem, groupByField: GroupByField): string {
  ...
  if (groupByField.field.type === 'date') {
    return (
      truncateDate(value as Date | null, groupByField.truncation ?? 'exact')?.toISOString() ??
      'null'
    );
  }
  return String(value);
