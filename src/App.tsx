      ...
      <TabLayout activeTab={activeTab} onTabChange={setActiveTab}>
        {activeTab === 'data' ? (
          <div className="h-full flex">
            <DataInput value={inputText} onChange={(data) => setInputText(data)} />
...
function flattenRecursively(obj: unknown, path: string[], result: Record<string, unknown>) {
  if (obj && typeof obj === 'object' && !Array.isArray(obj)) {
    for (const key in obj) {
      flattenRecursively((obj as Record<string, unknown>)[key], [...path, key], result);
    }
  } else {
...
function findArrays(obj: unknown) {
  ...
  function findArrayRecursively(obj: unknown, path: string[]) {
    if (Array.isArray(obj)) {
      ...
      for (const key in obj) {
        findArrayRecursively((obj as Record<string, unknown>)[key], [...path, key]);
      }
    }
  }
